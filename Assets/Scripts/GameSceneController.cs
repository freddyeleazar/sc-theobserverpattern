...
public class GameSceneController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        EventBroker.HitByEnemy += Ship_HitByEnemy;
    }
    ...
    private IEnumerator SpawnEnemies()
    {
        ...
        for (int i = 0; i < currentLevel.numberOfEnemies; i++)
        {
            ...
            EventBroker.EnemyDestroyed += Enemy_EnemyDestroyed;

            yield return wait;
        }
    }

    private void Enemy_EnemyDestroyed(int pointValue)//pointValue es asignado por el publisher (EnemyController) y es aprovechado por el suscriber (GameSceneController) para actualizar su variable totalPoints.
    {
        totalPoints += pointValue;
        EventBroker.CallScoreUpdateOnKill(totalPoints);
    }
